...
@Slf4j
@Component
@Service(interfaceClass=OrderServiceAPI.class, group="order_2018")
...
//		获取座位表文件名称
		String seatAddressName = moocOrder2018TMapper.getSeatAddress(fieldId);
...
//			若一次内部循环没找到，说明该座位无效，直接返回false
	...
	public boolean isSoldSeats(int fieldId, String soldSeats) {
		String allSoldSeats = moocOrder2018TMapper.getAllSoldSeats(fieldId);
...
//			若一次内部循环找到了一次，说明该座位已经被占，直接返回false
	...
	public OrderInfoVO getOrderInfoVOByOrderId(String uuid) {
		...
		return moocOrder2018TMapper.getOrderInfoVOByOrderId(uuid);
	}
	...
	public OrderInfoListVO getOrderInfoVOs(int userId, int nowPage, int pageSize) {
		...
		List<OrderInfoVO> orderInfoVOs = moocOrder2018TMapper.getOrderInfoVOs(userId);
...
//		插入订单信息，若成功，返回订单id
		int res = moocOrder2018TMapper.insert(moocOrderT);
		if(res > 0) {
	...
	public boolean paySuccess(String orderId) {
		...
		int res = moocOrder2018TMapper.updateById(moocOrderT);
		if(res > 0) {
			return true;
		}
