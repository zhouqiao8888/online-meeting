...
@Slf4j
@Component
@Service(interfaceClass=OrderServiceAPI.class, group="order_2019")
...
//		获取座位表文件名称
		String seatAddressName = moocOrderTMapper.getSeatAddress(fieldId);
...
//			若一次内部循环没找到，说明该座位无效，直接返回false
	...
	public boolean isSoldSeats(int fieldId, String soldSeats) {
		String allSoldSeats = moocOrderTMapper.getAllSoldSeats(fieldId);
...
//			若一次内部循环找到了一次，说明该座位已经被占，直接返回false
	...
	public OrderInfoVO getOrderInfoVOByOrderId(String uuid) {
		...
		return moocOrderTMapper.getOrderInfoVOByOrderId(uuid);
	}
	...
	public OrderInfoListVO getOrderInfoVOs(int userId, int nowPage, int pageSize) {
		...
		List<OrderInfoVO> orderInfoVOs = moocOrderTMapper.getOrderInfoVOs(userId);
...
//		插入订单信息，若成功，返回订单id
		int res = moocOrderTMapper.insert(moocOrderT);
		if(res > 0) {
...
//		采用令牌桶算法进行限流
	...
	public boolean paySuccess(String orderId) {
		...
		int res = moocOrderTMapper.updateOrderStatusById(orderId, 1);
		
//		MoocOrderT moocOrderT = new MoocOrderT();
//		moocOrderT.setUuid(orderId);
//		moocOrderT.setOrderStatus(1);
//		
//		int res = moocOrderTMapper.updateById(moocOrderT);
		if(res > 0) {
			return true;
		}
	...
	public boolean payFail(String orderId) {
		...
		int res = moocOrderTMapper.updateOrderStatusById(orderId, 2);
		if(res > 0) {
			return true;
		}
